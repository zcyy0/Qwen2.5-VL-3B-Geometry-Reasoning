...
On method, the goal is to fix the training signal so it rewards reasoning, not answers, and to transfer binding without the degeneration that killed vanilla distillation. Concretely: (1) Better on-policy distillation — supervise the teacher's token distribution on the student's own sampled trajectories rather than distilling teacher-generated sequences; distill only the compact binding step ("which theorem attaches to which diagram objects") rather than the full verbose trace; and prefer a style-closer teacher (a larger Qwen-VL) over Gemini/Claude to shrink the distribution gap. (2) Process- or verifier-level reward — reward the binding/derivation step, not just the final number, to break the stable-wrong-shortcut pattern the fluke audit surfaced; (3) Verifier-supplied "chosen" traces — gated DPO dropped 559 of 1289 problems for lacking any valid correct rollout; a verifier or teacher could generate valid derivations for those all-fluke problems, restoring learnable signal where quality-gating currently sees only flukes.
